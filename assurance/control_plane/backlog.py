...
current status; this validates it and generates everything anybody reads.
    ...
    python assurance/control_plane/backlog.py lint      schema, vocabulary and invariants
    python assurance/control_plane/backlog.py status    current phase and release readiness
    python assurance/control_plane/backlog.py graph     cycles and dependency ordering
...
from assurance.control_plane.evidence import (  # noqa: E402
    exact_outcome,
    load_result,
    validate_class_a,
...
def _deferral_report(doc: dict, *, as_of: date | None = None,
    ...
    if live:
        ...
        lines.append(f"{len(rows)} deferred row(s). Dates below are recorded obligations, "
                     "not cached current verdicts. Run "
                     "`python assurance/control_plane/backlog.py status` "
...
# consistent", and `assurance/specification/plan/export_current_plan.py` refuses to publish the
...
# step (`assurance/gate/check.py` "backlog population") and its own declared, exact,
...
#: The two rules, named once. `assurance/gate/known_failures.py` registers facts under
...
def population_report(groups: dict[str, list[tuple[str, str, str]]]) -> str:
    """The exact text the gate step prints and `assurance/gate/known_failures` parses.
...
def _structured_record(acid: str, level: str, ref: str) -> list[str]:
    ...
    THE RULES MOVED TO `assurance/control_plane/structured_evidence.py`. BK-80-AC1. This checked
    ...
    from assurance.control_plane.structured_evidence import problems
        ...
        for ac in item.get("acceptance") or []:
            ...
            for level, evidence in (ac.get("evidence") or {}).items():
                ...
                if level in AUTOMATED_EVIDENCE:
                    ...
                    from assurance.journeys.browser_evidence import load, problems, row_for
                    from assurance.journeys.journey import EXPECTED
...
def lint(doc: dict, *, verify_execution: bool = False) -> list[str]:
    ...
    from assurance.control_plane.release_obligations import mapping_problems
    ...
    from assurance.control_plane.readiness_plan import problems as readiness_problems
...
def _missing_pytest(acid: str, ref: str, *,
    ...
    false failures when `assurance/gate/known_failures.py` first ran, and here it fails
...
def board(doc: dict, *, bind_execution: bool = True,
    ...
    lines = [
        ...
        "",
        "**Generated by `python assurance/control_plane/backlog.py render`. Do not edit by "
...
def main() -> int:
    ...
    if args.command == "obligations":
        if not args.item:
            ...
            return 2
        from assurance.control_plane.release_obligations import obligations
