"""Compare the failures a gate run actually produced against the declared ones.
    ...
    from assurance.gate.known_failures import load, observed, compare
...
----------------------------------
`assurance/gate/check.py` returns 1 on any failure and writes no stamp, so
`assurance/hooks/pre-commit` refuses the commit. With three unbuilt PRD obligations
...
registry changes -- see `assurance/control_plane/evidence.py`.
"""
...
_PYLINT_FACT = re.compile(
    r"^(.+?):(\d+):(\d+):\s+([A-Z]\d{4}):\s+(.+?)\s*$")
#: `assurance/control_plane/backlog.py population`: `  [group] member  optional note`. The member
...
class Verdict:
    ...
    def ok(self) -> bool:
        ...
        red; `assurance/gate/check.py` prints FULL GATE RED on this path.
        """
