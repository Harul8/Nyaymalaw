"""The one command. Run it after every task, before claiming anything is done.
    ...
    python assurance/gate/check.py            # the per-task gate
    python assurance/gate/check.py --slice 1  # add the golden suite runnable at slice N
...
explicit per-run approval. `assurance/gate/check.py` must stay cheap enough that there is
...
from assurance.common.homes import TOOLING as TOOLING_HOMES  # noqa: E402
...
def _scoped_verdict(failed: list[str], captured: dict[str, str],
    ...
    try:
        from assurance.gate.gatestamp import record
...
def _known_failure_registry_is_empty() -> bool:
    ...
    """
    try:
        from assurance.gate.known_failures import load
...
def main() -> int:
    ...
    results = []
    ok, _ = step("layercheck", [py, "assurance/gate/layercheck.py"])
    results.append(("layercheck", ok))
    prints.append(("layercheck", verification_fingerprint()))
    ok, _ = step("export_spec", [py, "assurance/gate/export_spec.py"])
    results.append(("export_spec", ok))
    prints.append(("export_spec", verification_fingerprint()))
    ok, out = step("trace", [py, "assurance/gate/trace.py", "--skip-regen"])
    captured["trace"] = out
    ...
    ok, out = step("backlog population", [py, "assurance/control_plane/backlog.py", "population"])
    ...
    #
    # WHAT THAT COSTS: a regression in `frontend/` -- the stylesheet, the page or
    ...
    #
    # Run it after any change under `frontend/`. `tests/test_no_css_class_has_two
    ...
    if args.slice is not None:
        ...
        ok, _ = step("goldens structure + authority",
                     [py, "assurance/journeys/run_goldens.py"])
        results.append(("goldens", ok))

        ok, _ = step(f"slice gate S{args.slice}",
                     [py, "assurance/gate/slicegate.py", "--slice", str(args.slice)])
        results.append(("slicegate", ok))

        if args.scenarios:
            ok, _ = step(f"scenarios slice-{args.slice}",
                         [py, "assurance/journeys/run_scenario.py", "--approve",
    ...
    # `assurance/specification/plan/build_plan.py` that did not parse reached HEAD. Recording the
    # digest here is what lets `assurance/gate/gatestamp.py` -- and the pre-commit hook
