"""Is slice N done? The plan's own rule, made runnable.
    ...
    python assurance/gate/slicegate.py            # every slice
    python assurance/gate/slicegate.py --slice 2  # one
...
GOLDEN_RUNNER = ROOT / "assurance" / "journeys" / "run_goldens.py"
...
def load() -> tuple[list[dict], list[dict], set[str], set[str]]:
    ...
    ev = yaml.safe_load((spec / "evals.yaml").read_text(encoding="utf8"))["evals"]
    fs = yaml.safe_load((spec / "features.yaml").read_text(encoding="utf8"))["features"]
...
def main() -> int:
    ...
    if not GOLDEN_RUNNER.exists():
        print()
        print("  THE BLOCKER: assurance/journeys/run_goldens.py does not exist. Every slice's")
