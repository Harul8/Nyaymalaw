"""Did the gate pass on THIS tree, or on one that no longer exists?
    ...
    python assurance/gate/gatestamp.py            # is the tree the one the gate passed on?
`assurance/gate/check.py` is the only command that records a pass. This reader cannot
turn its own invocation into evidence.

WHAT HAPPENED, 6 September 2026
--------------------------------
A heredoc collapsed `\\n\\n` inside a nested string literal, `assurance/specification/plan/
...
#: is IDENTITY_MANIFEST in assurance.control_plane.evidence, not a second list in this module.
...
def state(*, require_index: bool = False) -> tuple[str, str]:
    ...
    if was == now:
        kind = stamp["kind"]
        if kind == "scoped":
            from assurance.gate.known_failures import load, registry_digest
...
def main() -> int:
    ...
    print()
    print("  Run `python assurance/gate/check.py`, or commit with --no-verify if you")
