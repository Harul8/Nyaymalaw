"""The three-way diff: spec <-> code <-> eval results.
    ...
    python assurance/gate/trace.py            # report and exit non-zero on any failure
    python assurance/gate/trace.py --summary  # counts only
...
def load_spec() -> tuple[list[dict], list[dict]]:
    ...
    if not fpath.exists() or not epath.exists():
        sys.exit("spec not generated -- run: python assurance/gate/export_spec.py")
...
def spec_is_current(rep: Report) -> None:
    ...
    try:
        from assurance.gate.export_spec import build, compare_payloads
    ...
    for path, condition in stale:
        rep.fail("T1", f"{path.relative_to(ROOT)} is {condition} -- nothing was "
                       "rewritten. Run `python assurance/gate/export_spec.py --write` "
