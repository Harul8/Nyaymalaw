...
A(table(
  ...
  [
    ...
    ['`para_kind`', 'attributable · not_attributable · unknown', 'Counsel\'s submission can never be quoted as the holding.'],
...
const controls = [
  ...
  ['**H7 · Attribution discipline**',
   ...
   'A proposition attributed to a judgment resolves to a paragraph the corpus classifies as the court deciding, reasoning or ordering — carried as `para_kind = attributable`. A `not_attributable` paragraph (a submission by counsel, a headnote, a recital of facts) may be **quoted with its status disclosed** and may never carry a proposition. `unknown` is a paragraph nobody classified, and is treated as the third state and not as either answer. **Reliance on obiter is labelled as obiter — and NOTHING IMPLEMENTS THIS TODAY:** the corpus carries no obiter class, and the `ratio`/`reasoning` split it used to be read out of was measured unreliable and collapsed. Recorded as owed, not as met.',
   'Class B — `para_kind` is non-optional on every Finding and is checked at use, and `tests/test_attributable_has_one_owner.py` refuses a second definition of which labels qualify. Class C — the unclassified share (26.7%) is reported, and any classification pass reports its residual. The obiter clause has NO eval, because it has no mechanism.'],
...
A(table(
  ...
  [
    ...
    ['**P5 · Every gate declares its own response**', 'A condition that refuses something declares WHAT it refuses (turn, thread, step or evidence need) and HOW (withhold, block, or disclose). Those declarations live in one table — §7.1A, generated from `backend/nm/domain/gates.py` — and no call site decides for itself.', 'The first draft of this document said the product *fails closed only on grounding* while nine conditions elsewhere in it blocked something. Both statements were written in good faith and they cannot both be true. **A prose rule about what happens when things go wrong will always drift from the code that handles it**, so the rule is now a table the code reads and the document renders.'],
