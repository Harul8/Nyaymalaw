...
function render(A) {
  ...
  A(H.p('An external review found nine of these contracts unable to hold what their own DOES clause required. `WitnessPlan` was a sentence of prose; `DrafterBrief` was a pointer; `ClosureRecord` had no fields at all; and `CaseSummary` was never defined anywhere although two features derive from it. They are set out in full below, and `assurance/gate/speccheck.py` refuses a PRODUCES clause that contradicts one.'));
