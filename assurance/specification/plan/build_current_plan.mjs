...
if (!python || !runtime) throw new Error('Provide --python and --runtime-path. See assurance/specification/plan/README.md.');
...
const data = JSON.parse(execFileSync(python, [path.join(root, 'assurance/specification/plan/export_current_plan.py')], {
  cwd: root, encoding: 'utf8', maxBuffer: 30 * 1024 * 1024,
}));
const context = JSON.parse(await fs.readFile(path.join(root, 'assurance/specification/plan/view_content.json'), 'utf8'));
...
const readme = [
  ...
  ['Scenario and risk catalogues','JS-, R- and G-labels organise preserved design examples in assurance/specification/plan/view_content.json. They are not extra work items or executed proofs. Each points to registered delivery work; execution coverage must be recorded there.'],
...
sourceRows.push(['9 September 2026 end-to-end workbook','Historical plan source','Preserved unchanged','Supplementary design context is now tracked in assurance/specification/plan/view_content.json; no historical verification counts carry forward.']);
...
reconRows.push(['Structural lint problems',0,data._snapshot.structural_lint_problems,data._snapshot.structural_lint_problems,'assurance/control_plane/backlog.py lint','Does not certify manual wave or release-profile gates.']);
