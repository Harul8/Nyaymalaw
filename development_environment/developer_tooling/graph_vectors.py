...
in `.claude/settings.json` and `assurance/hooks/pre-commit` -- ran `update` and
...
def _credentials() -> dict[str, str]:
    """The same three facts `development_environment/developer_tooling/crg_serve.ps1`
    sets, read the same way.
...
def report() -> None:
    ...
    try:
        ...
        print(f"SEMANTIC INDEX: {exc} -- semantic search cannot answer at all; "
              ...
              f"development_environment/developer_tooling/graph_vectors.py --embed`")
        return
    if behind == 0:
    ...
    print("  Fix: python development_environment/developer_tooling/graph_vectors.py --embed")
