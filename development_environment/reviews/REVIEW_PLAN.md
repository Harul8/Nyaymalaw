...
| area | files | lines |
|---|---:|---:|
| `backend/nm/domain` | 24 | 5,606 |
| `backend/nm/ports` | 8 | 1,265 |
| `backend/nm/knowledge` | 9 | 1,753 |
| `backend/nm/core` | 25 | 11,541 |
| `backend/nm/adapters` | 16 | 3,616 |
| `backend/nm/edge` | 3 | 991 |
| `backend/nm/bootstrap` | 3 | 218 |
| `tests/` | 93 | 26,062 |
| `tools/` | 26 | 8,633 |

**`backend/nm/core/turn.py` is 4,536 lines — 18% of the product in one file**, and it
...
1. `python assurance/gate/check.py` on a **quiet tree**; record the fingerprint. A review
...
### Phase 1 — Contracts · `backend/nm/domain` + `backend/nm/ports` (6.9k) ☐
...
### Phase 2 — Legal knowledge · `backend/nm/knowledge` (1.7k) ☐
...
### Phase 3 — `backend/nm/core`, by turn phase, not by file (11.5k) ☐
...
### Phase 4 — `backend/nm/adapters` (3.6k) ☐
...
### Phase 5 — `backend/nm/edge` + `backend/nm/bootstrap` (1.2k) ☐
