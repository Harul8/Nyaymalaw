...
* attachment needs a resolved locator and verbatim words; a ranked snippet, a
...
def test_a_resolved_verbatim_passage_attaches_with_five_separate_verdicts(tmp_path):
    ...
    assert a["source_version"], "the attachment names no source version"
...
def test_a_snippet_shaped_locator_is_refused_as_unresolved(tmp_path):
    ...
    assert research["attachments"] == [], "a refused attachment reached the record"


def test_a_case_this_research_did_not_surface_cannot_be_attached_through_it(tmp_path):
    """Scope on attachment: the locator is real and the words verbatim, and
...
def test_a_correct_quote_from_an_overruled_case_keeps_its_negative_treatment(tmp_path):
    ...
    overruled decision.* The attachment is a verified CITATION -- identity
...
def test_a_withdrawn_source_version_marks_the_attached_input_withdrawn(tmp_path):
    ...
    assert attachment["ledger_id"].endswith(":SYN_1990_MARKER_P002_C01")
    assert attachment["source_version"] == "synthetic-2026-09-12"

    before = client.get(f"/api/matters/{matter_id}/dependencies").json()
    row = next(t for t in before["tracked"] if t["id"] == attachment["ledger_id"])
    assert row["withdrawn"] is False and row["version"] == 1

    # THE WITHDRAWAL. The next turn re-reads the file against it.
    evidence.gone = frozenset({attachment["source_version"]})
    ...
    row = next(t for t in after["tracked"] if t["id"] == attachment["ledger_id"])
