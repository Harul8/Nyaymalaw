...
def _inputs():
    ...
    criteria = {
        criterion["id"] for item in registry["items"] for criterion in item.get("acceptance", [])
    }
...
def _target(root, path):
    ...
    for key in path[:-1]:
        assert (
            key in node if isinstance(node, dict) else isinstance(key, int) and 0 <= key < len(node)
        )
        node = node[key]
    key = path[-1]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            "media contract.attribution_is_authentication",
        ),
        (
            ("media_contract", "procurement", "scope"),
            "entire_vendor",
            "media contract.procurement.scope",
        ),
        (
            ("media_contract", "procurement", "hidden_processing"),
            "ignore",
            "media contract.procurement.hidden_processing",
        ),
        (
            ("media_contract", "procurement", "unavoidable_prohibited_processing"),
            "allow",
            "media contract.procurement.unavoidable_prohibited_processing",
        ),
        (
            ("media_contract", "procurement", "unrelated_optional_vendor_services"),
            "reject_vendor",
            "media contract.procurement.unrelated_optional_vendor_services",
        ),
        (
            ("media_contract", "request", "check_before_bytes"),
            False,
            "media contract.request.check_before_bytes",
        ),
        (
            ("media_contract", "request", "unknown_configuration"),
            "allow",
            "media contract.request.unknown_configuration",
        ),
        (
            ("media_contract", "request", "consent_override"),
            True,
            "media contract.request.consent_override",
        ),
        (
            ("media_contract", "response", "validation"),
            "top_level_only",
            "media contract.response.validation",
        ),
        (
            ("media_contract", "response", "unknown_or_forbidden_fields"),
            "log_then_drop",
            "media contract.response.unknown_or_forbidden_fields",
        ),
        (
            ("media_contract", "response", "raw_rejected_response_persistence"),
            True,
            "media contract.response.raw_rejected_response_persistence",
        ),
        (
            ("media_contract", "response", "protected_sinks"),
            ["ui"],
            "media contract.response.protected_sinks",
        ),
        (
            ("media_contract", "evidence", "originals"),
            "rewrite",
            "media contract.evidence.originals",
        ),
        (
            ("media_contract", "evidence", "legal_assessment"),
            "ban",
            "media contract.evidence.legal_assessment",
        ),
        (
            ("media_contract", "observations", "missing"),
            "PASS",
            "media contract.observations.missing",
        ),
        (
            ("media_contract", "observations", "equality"),
            "coerce",
            "media contract.observations.equality",
        ),
        (
            ("media_contract", "observations", "derive_from_expected"),
            True,
            "media contract.observations.derive_from_expected",
        ),
        (
            ("media_contract", "observations", "allowed_transcription"),
            "no_observation_needed",
            "media contract.observations.allowed_transcription",
        ),
        (
            ("media_contract", "observations", "denied_request"),
            "hidden_ui",
            "media contract.observations.denied_request",
        ),
    ],
)
...
@pytest.mark.parametrize(
    "case_id, path, replacement, complaint",
    [
        ("EVAL-007", ("owner_criteria",), [], "EVAL-007: media acceptance ownership incomplete"),
        (
            "EVAL-007",
            ("inputs", "requested_operation"),
            "voice_identity",
            "EVAL-007 allowed request",
        ),
        ("EVAL-007", ("inputs", "prohibited_request_variants"), [], "EVAL-007 prohibited requests"),
        (
            "EVAL-008",
            ("inputs", "forbidden_response_fields"),
            ["emotion"],
            "EVAL-008 response field population",
        ),
        (
            "EVAL-008",
            ("inputs", "response_mutation_paths"),
            ["metadata"],
            "EVAL-008 response mutation paths",
        ),
        (
            "EVAL-008",
            ("inputs", "provider_response", "segments"),
            [],
            "EVAL-008: response mutation containers must exist",
        ),
        (
            "EVAL-008",
            ("inputs", "provider_response", "metadata", "operation"),
            "voice_identity",
            "EVAL-008: allowed provider transcript fixture missing",
        ),
        (
            "EVAL-008",
            ("inputs", "frames"),
            [],
            "EVAL-008: original media and transcript fixtures must be nonempty",
        ),
        (
            "EVAL-008",
            ("inputs", "transcript"),
        ...
        (
            "EVAL-008",
            ("inputs", "transcript", 0, "text"),
            "Different unobserved text",
            "EVAL-008: provider response and ordinary transcript disagree",
        ),
        (
            "EVAL-008",
            ("inputs", "response_mutation_value"),
            "",
            "EVAL-008: forbidden response canary missing",
        ),
        (
            "EVAL-008",
            ("inputs", "observation_mutations"),
            [],
            "EVAL-008: missing-observation controls absent",
        ),
        (
            "EVAL-008",
            ("inputs", "observation_mutations", 0, "path"),
            "nonexistent",
            "EVAL-008: missing-observation control population changed",
        ),
        (
            "EVAL-008",
            ("inputs", "observation_mutations", 3, "path"),
            "nonexistent",
            "EVAL-008: missing-observation control population changed",
        ),
        (
            "EVAL-008",
            ("inputs", "observation_mutations", 4, "action"),
            "ignore",
            "EVAL-008: missing-observation control population changed",
        ),
        (
            "EVAL-027",
            ("inputs", "processor_configuration", "hidden_prohibited_processing"),
            True,
            "EVAL-027 selected configuration.hidden_prohibited_processing",
        ),
        (
            "EVAL-027",
            ("inputs", "processor_configuration", "vendor_offers_unrelated_optional_biometrics"),
            False,
            "EVAL-027 selected configuration.vendor_offers_unrelated_optional_biometrics",
        ),
        (
            "EVAL-027",
            ("inputs", "configuration_mutations"),
            [],
            "EVAL-027: configuration controls absent",
        ),
        (
            "EVAL-027",
            ("inputs", "configuration_mutations", 0, "field"),
            "nonexistent",
            "EVAL-027: configuration control population changed",
        ),
    ],
)
...
def test_media_success_and_denial_observations_cannot_disappear(case_id, path):
    ...
    errors = check_evaluations(catalog, criteria)
    assert (
        f"{case_id} media observation {path}: required observation missing or duplicated" in errors
    )
...
def test_false_observation_is_not_missing_zero_string_or_true(replacement):
    ...
    expected = next(
        row
        for row in _case(catalog, "EVAL-008")["expected"]
    ...
    errors = check_evaluations(catalog, criteria)
    assert (
        "EVAL-008 media observation transcript.speaker_authenticated: required typed policy changed"
        in errors
    )
...
def test_zero_leaks_do_not_hide_empty_or_incomplete_sink_inspection(inspected):
    ...
    for path in (
        "forbidden_response.downstream_occurrences",
        "forbidden_response.uninspected_sinks",
    ):
    ...
    errors = check_evaluations(catalog, criteria)
    assert (
        "EVAL-008 media observation forbidden_response.inspected_sink_categories: "
        "required policy population changed"
        in errors
    )
