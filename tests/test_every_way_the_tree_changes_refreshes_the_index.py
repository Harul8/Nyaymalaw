...
--------------------------------------
`assurance/hooks/pre-commit` had refreshed the vectors on every `git commit`
...
3. The hooks are the tracked directory itself (`core.hooksPath assurance/hooks`),
...
`development_environment/developer_tooling/graph_vectors.py --check` at commit, merge and session
start, where
...
def test_each_way_the_tree_changes_has_a_hook_that_refreshes(name):
    ...
    assert (HOOKS / name).exists(), (
        f"assurance/hooks/{name} is missing, so a tree changed by that path "
        f"leaves the graph describing the previous one")
    code = "\n".join(_code_lines(_hook(name)))
    assert f"assurance/hooks/{OWNER}" in code, (
    ...
    for path in HOOKS.iterdir():
        ...
        for pattern in REFRESH_COMMANDS:
            assert not pattern.search(code), (
                f"assurance/hooks/{path.name} carries `{pattern.pattern}` "
...
def test_the_refresh_is_not_duplicated_outside_the_hooks_either():
    ...
    assert homes == {f"assurance/hooks/{OWNER}"}, (
        f"`graph_vectors.py --embed` is invoked from {sorted(homes)}; "
        f"one owner, and it is assurance/hooks/{OWNER}")
...
def test_every_hook_is_tracked_as_executable(name):
    row = subprocess.run(
        ["git", "ls-files", "--stage", f"assurance/hooks/{name}"],
        cwd=ROOT, capture_output=True, text=True, check=True,
    ).stdout.strip()
    assert row.startswith("100755 "), (
        f"assurance/hooks/{name} is not tracked as 100755 -- a POSIX checkout "
...
def test_every_hook_stays_lf_through_a_windows_checkout(name):
    ...
    attrs = subprocess.run(
        ["git", "check-attr", "eol", "--", f"assurance/hooks/{name}"],
        cwd=ROOT, capture_output=True, text=True, check=True,
    ).stdout.strip()
    assert attrs.endswith(": eol: lf"), (
        f"assurance/hooks/{name} has no `eol=lf` attribute ({attrs!r}); a Windows "
        f"checkout can rewrite it CRLF and sh will not run it")
    raw = (HOOKS / name).read_bytes()
    assert b"\r" not in raw, f"assurance/hooks/{name} already carries CRLF"
...
def test_the_hooks_are_installed_by_path_not_by_copy():
    ...
    assert "core.hooksPath assurance/hooks" in text, (
        "pre-commit no longer documents `git config core.hooksPath "
        "assurance/hooks`, so the next install is a copy that can drift")
