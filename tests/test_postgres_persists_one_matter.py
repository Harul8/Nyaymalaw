...
def store(tmp_path):
    ...
    made = PostgresMatterStore(connect=connect, sealer=sealer,
                               workspace_id=WS_A)
    made.create_schema()
    yield made
    with made._tx() as cur:
        for table in ("nm_outbox", "nm_operation", "nm_matter"):
            cur.execute(f"DELETE FROM {table} WHERE workspace_id IN (%s, %s)",
                        (WS_A, WS_B))


@pytest.fixture
def other(store, tmp_path):
    return PostgresMatterStore(connect=store.connect,
                               sealer=MatterSealer(SEAL, tmp_path / "keys"),
                               workspace_id=WS_B)


def _matter(advocate: str = "adv@example.test") -> Matter:
    return Matter(id=f"mat_{uuid.uuid4().hex[:12]}", advocate_id=advocate,
...
def _operation(matter: Matter, key: str, workspace: str = WS_A) -> Operation:
    return Operation(
        ...
        advocate_id=matter.advocate_id, command="create-matter",
...
def test_the_target_database_is_disposable():
    ...
    assert any(word in name for word in ("test", "scratch", "tmp", "disposable",
                                         "synthetic", "nm_dev")), (
...
def test_one_accepted_command_writes_the_matter_the_operation_and_the_outbox(
        store):
    matter = _matter()
    entry = OutboxEntry(entry_id=f"e_{uuid.uuid4().hex[:8]}",
                        workspace_id=WS_A, operation_key="key-1",
                        kind="advise", matter_id=str(matter.id))

    saved = store.commit_accepted(matter, expected_version=0,
                                  operation=_operation(matter, "key-1"),
                                  outbox=(entry,))
...
def test_a_refused_outbox_entry_leaves_no_matter_behind(store):
    ...
    matter = _matter()
    bad = OutboxEntry(entry_id="e-bad", workspace_id=WS_B,
                      operation_key="key-2", kind="advise")
    with pytest.raises(OutboxRefused):
        store.commit_accepted(matter, expected_version=0,
                              operation=_operation(matter, "key-2"),
                              outbox=(bad,))
...
def test_a_stale_writer_is_refused_and_the_winner_stands(store):
    matter = _matter()
    first = store.commit_accepted(matter, expected_version=0,
                                  operation=_operation(matter, "k-a"))
    store.commit_accepted(first, expected_version=1,
                          operation=_operation(matter, "k-b"))

    with pytest.raises(StaleWrite) as refused:
        store.commit_accepted(first, expected_version=1,
                              operation=_operation(matter, "k-c"))
...
def test_a_replayed_key_returns_the_original_and_does_no_second_work(store):
    ...
    matter = _matter()
    store.commit_accepted(matter, expected_version=0,
                          operation=_operation(matter, "same-key"))

    again = store.commit_accepted(matter, expected_version=0,
                                  operation=_operation(matter, "same-key"))
    assert again.id == matter.id
    assert store.load(matter.id).version == 1, "the replay did the work twice"

    with store._tx() as cur:
        cur.execute("SELECT count(*) FROM nm_matter WHERE workspace_id = %s",
                    (WS_A,))
...
def test_the_same_key_with_a_different_request_is_a_conflict(store):
    matter = _matter()
    store.commit_accepted(matter, expected_version=0,
                          operation=_operation(matter, "reused"))
    changed = Operation(
        idempotency_key="reused", workspace_id=WS_A,
        advocate_id=matter.advocate_id, command="delete-matter",
        matter_id=str(matter.id), request_digest="a different request")
...
def test_workspace_b_cannot_read_or_list_workspace_as_matter(store, other):
    matter = _matter()
    store.commit_accepted(matter, expected_version=0,
                          operation=_operation(matter, "k-iso"))
...
def test_workspace_b_cannot_update_workspace_as_matter(store, other):
    matter = _matter()
    saved = store.commit_accepted(matter, expected_version=0,
                                  operation=_operation(matter, "k-upd"))
...
def test_one_key_in_two_workspaces_are_two_operations(store, other):
    ...
    store.commit_accepted(mine, expected_version=0,
                          operation=_operation(mine, "collide"))
    other.commit_accepted(theirs, expected_version=0,
                          operation=_operation(theirs, "collide", WS_B))

    assert store.operation(WS_A, "collide").matter_id == str(mine.id)
    assert other.operation(WS_B, "collide").matter_id == str(theirs.id)


def test_a_reused_connection_carries_no_tenant_context_between_transactions(
        store, other):
    ...
    matter = _matter()
    store.commit_accepted(matter, expected_version=0,
                          operation=_operation(matter, "k-pool"))
    for _ in range(3):
...
def test_every_persisted_field_survives_a_round_trip(store):
    ...
    matter = _matter()
    store.commit_accepted(matter, expected_version=0,
                          operation=_operation(matter, "k-fields"))
    back = store.load(matter.id)
    before = _enc(matter)
    after = _enc(back)
    before.pop("version", None)
    after.pop("version", None)
    assert json.loads(json.dumps(after, default=str)) == \
        json.loads(json.dumps(before, default=str))
