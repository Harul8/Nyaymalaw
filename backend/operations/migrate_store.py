"""Rehearsing a store cutover, and refusing one that cannot be shown safe.
    ...
    python backend/operations/migrate_store.py rehearse --source /tmp/source --target /tmp/target \
        --synthetic-quiesced
    python backend/operations/migrate_store.py reconcile --source .nm --target /tmp/target
