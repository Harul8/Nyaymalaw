...
have an owner:
    ...
    the durable job, its lease, its retries      backend/nm/core/worker.py (P11)
    what became of it, including UNKNOWN         backend/nm/domain/operation.py
    who may act at all                           backend/nm/domain/authority.py
    the conflict screen itself                   backend/nm/core/conflict.py
...
--------------------------------------------
`backend/nm/core/conflict.py` finds the clash. What it may TELL the advocate about
