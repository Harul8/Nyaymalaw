...
-----------------------------------------------
It began in `assurance/common/_fingerprint.py`, so that a recorded mutation run could not
...
This is the same argument `backend/nm/knowledge/artefact.py` makes about the dense
...
FINGERPRINTED = ("backend/nm", "tests")
...
def source_fingerprint(root: Path | None = None) -> str:
    ...
    for top in FINGERPRINTED:
        ...
        if not base.exists():
            ...
            # A deployment that ships `backend/nm/` without `tests/` is ordinary; a
