...
`backend/nm/bootstrap/composition._refuse_a_shared_seal` calls it rather than
...
def shared_values(values: dict[str, str]) -> tuple[tuple[str, ...], ...]:
    ...
    THE ONE OWNER OF THAT QUESTION. `backend/nm/bootstrap/composition` asks it about
...
#: test somebody runs. `assurance/control_plane/backlog.py` reads them from a JSON document and
...
class EvidenceRecord:
    ...
    PASS. ``assurance.control_plane.structured_evidence`` owns the authoritative schema and trust
