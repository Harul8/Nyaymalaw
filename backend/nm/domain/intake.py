...
---------------------------------------------------
    ...
    ADMITTED       `backend/nm/domain/media.py` decided its quarantine and purpose
...
------------------------------
It does not decide whether material may be processed -- `backend/nm/domain/media_
policy.py` owns that and this module calls it. It does not store bytes;
`backend/nm/adapters/store` does. It holds the RECORD of what arrived and what was
made of it.
"""
...
class ReadQuality(str, Enum):
    ...
    NOT `Certainty`, AND THE NAME MATTERS. `backend/nm/domain/matter.Certainty` is
