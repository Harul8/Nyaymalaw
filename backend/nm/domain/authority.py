...
command, a background job before it publishes, `backend/nm/domain/decision.py` before
...
------------------------------------------------------
`backend/nm/domain/matter.Role` is the LITIGATION POSTURE -- plaintiff, defendant,
...
class ActingAs(str, Enum):
    ...
    NOT `Capacity`, AND THE NAME MATTERS. `backend/nm/core/screens.Capacity` already
    ...
    ALSO NOT `backend/nm/domain/matter.Role`, which is the litigation posture.
...
def permits(actor_id: str, capacity: ActingAs, act: Act) -> Ruling:
    ...
    and `backend/nm/domain/decision.py` asks before recording. None of them decides
