...
`tests/test_blank_values.py` walks every dataclass in `backend/nm/` and fails the build
...
def fold(text: str | None) -> str:
    ...
    MEASURED, 6 September 2026, on the six definitions then in `backend/nm/`:

        backend/nm/core/chronology.py   words only  ── the same three characters
        backend/nm/core/dispute.py      words only
        backend/nm/core/posture.py      words only
        backend/nm/core/grounding.py    words only, plus the case-name `vs`/`v` pivot
        backend/nm/domain/issue.py      WHITESPACE COLLAPSE ONLY — punctuation kept
        backend/nm/domain/decision.py   WHITESPACE COLLAPSE ONLY
    ...
    `tests/test_one_fold.py` scans `backend/nm/` and fails the build on a second
...
def fold_spacing(text: str | None) -> str:
    ...
    module exists: `backend/nm/core/research.py` defined it privately and
...
def snippet(text: str | None, limit: int) -> str:
    ...
    `backend/nm/` cut prose the same way.
    ...
    * IT DOES NOT SAY IT CUT. A shortened statement rendered without a mark
    ...
    `tests/test_prose_is_cut_on_words.py` scans `backend/nm/` and fails the build on a
