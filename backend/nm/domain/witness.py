...
absence of the field, exactly as `backend/nm/core/proof.py` argues for its own tripwire.
...
-------------------------------
Who may concede, and on whose authority, is `backend/nm/domain/authority.py`
(`permits`, `Act.CONCEDE`) over `backend/nm/domain/commission.py`'s instructing/deciding
split. A second copy of that here would be the defect CLAUDE.md §4 asks about,
so the negotiation boundary is DERIVED in `backend/nm/core/hearing.py` and no
...
#: `assurance/specification/schemas.yaml` rather than trusting it as a copy.
