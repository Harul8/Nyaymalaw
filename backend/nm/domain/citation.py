...
------------------------------------------------------
`backend/nm/core/grounding.py` had a pattern for reading provision references out of
emitted text. `backend/nm/adapters/evidence/corpus.py` had a different one for reading
...
def reporter_key(raw: str) -> str:
    ...
    `pipeline/indexing/build_identity_index.py` writes `citations.citation_key` with this
