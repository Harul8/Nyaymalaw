"""Is the CORPUS wrong, or is the classifier? Only a third party can say.
    ...
    python tools/audit_labels.py --worksheet          # sample, read, list disputes
    python tools/audit_labels.py --score              # after you have adjudicated

WHY THIS EXISTS
-----------------
`tools/classify_paragraphs.py` measured a model against the corpus's own
...
from tools.classify_paragraphs import (  # noqa: E402
    ATTRIBUTABLE,
    KINDS,
    classify,
    sample_from_cases,
...
def worksheet(sample: int, cases: int, court: str) -> int:
    ...
    print()
    print("  Fill in the VERDICT lines, then: python tools/audit_labels.py "
          "--score")
    return 0
