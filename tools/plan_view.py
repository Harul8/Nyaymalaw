...
SHEETS = {
    "Read Me", "Overview", "Delivery Plan", "Journey Steps", "Step Contracts",
    "Features", "Work Items", "Traceability", "Build Assurance", "Release Gates",
    "Release Profiles", "Journey Scenarios", "Risks", "Advocate Standard",
    "Expert Workflow", "Advice Maturity", "Roles & Authority", "Gap Closure",
    "Modules", "Execution Packets", "Packet Guide", "Acceptance", "Decisions",
    "Command Contracts", "Evaluation Specs", "Evaluation Details", "Sources",
    "Reconciliation",
}
...
SNAPSHOT_BASE = {
    "docs/backlog/status.yaml", "docs/backlog/steps.yaml", "docs/backlog/plan.json",
    "docs/backlog/professional.json", "docs/backlog/build_rules.json",
    "docs/backlog/SCHEMA.md", "docs/BUILD_GUIDE.md",
...
CONTRACT_ASPECTS = {
    "actor": "Actor", "entry_conditions": "Entry conditions", "user_action": "User action",
    "expected_visible_result": "Visible result", "expected_domain_effect": "Domain effect",
    "failure_behaviour": "Failure / refusal", "recovery_behaviour": "Recovery / return",
    "exit_conditions": "Exit conditions", "notes": "Context",
}
    ...
    try:
        for sheet in values:
            ...
            if len(rows) < 3:
                ...
                continue
            headers = list(rows[2])
            if (not headers or any(not isinstance(v, str) or not v.strip() for v in headers)
            ...
            for excel_row, row in enumerate(rows[3:], 4):
                ...
                if len(row) > len(headers) and any(v is not None for v in row[len(headers):]):
                    result["__load_errors__"].append({"message": f"{sheet.title}:{excel_row}: data outside headers"})
                result[sheet.title].append({h: row[i] if i < len(row) else None
                                            for i, h in enumerate(headers)})
            for cells in formulas[sheet.title].iter_rows():
                for cell in cells:
                    if cell.data_type == "f":
                        cached = (rows[cell.row - 1][cell.column - 1]
                                  if cell.row <= len(rows) and cell.column <= len(rows[cell.row - 1])
                                  else None)
                        result["__formulas__"].append({"sheet": sheet.title,
                            "cell": cell.coordinate, "formula": cell.value, "cached": cached})
    finally:
        values.close()
        formulas.close()
    return result
...
def source_hash(path: Path) -> str:
    content = (path.read_text(encoding="utf-8").replace("\r\n", "\n").encode()
               if path.suffix in TEXT_SUFFIXES else path.read_bytes())
    return hashlib.sha256(content).hexdigest()


def _read(root: Path, relative: str):
    text = (root / relative).read_text(encoding="utf-8")
    return yaml.safe_load(text) if relative.endswith(".yaml") else json.loads(text)
...
def check_view(tables: dict[str, list[dict]], root: Path = ROOT) -> list[str]:
    ...
    errors.extend(row.get("message", "workbook: unreadable row")
                  for row in tables["__load_errors__"])
    for name in SHEETS:
    ...
    context = _read(root, "spec/plan/view_content.json")
    ...
    acceptance = [{**ac, "item": item["id"]} for item in items for ac in item.get("acceptance") or []]
    ...
    def identities(sheet: str, column: str, source: list[dict]) -> None:
        ...
        if actual != expected:
            ...
            errors.append(f"{sheet}: exact ID population/order differs; missing={missing}, extra={extra}")
    ...
    for row in items:
        ...
        compare("Work Items", "ID", row["id"], {
            "Next action": next_action,
            "Title": row["title"], "Kind": row["kind"], "Priority": row["priority"],
            "Delivery status": row["delivery_status"], "Implementation": row["implementation"],
            "Authored verification": row["verification"], "Wave": wave[row["id"]],
            "Hard dependencies": _list(row.get("depends_on")),
            "Acceptance count": len(row.get("acceptance") or []), "Record": row["record"],
        })
    for row in features:
        compare("Features", "Feature", row["id"], {
            "Phase": row["phase"], "Title": row["title"], "Build slice": row["slice"],
            "Implementation": row["implementation"], "Disposition": row["disposition"],
            "Delivery items": _list(row.get("delivery_items")),
        })
    for row in steps:
        compare("Journey Steps", "Step ID", row["id"], {
            "Phase": row["phase"], "Journey step": row["name"], "Basis": row["basis"],
            "Features": _list(row["features"]), "Registered work": _list(row["items"]),
    ...
    for row in acceptance:
        ...
        compare("Acceptance", "Criterion", row["id"], {
            "Work item": row["item"], "Required behaviour": row["requirement"],
            ...
            "Final packet": _list([p["id"] for p in contributors
                                     if row["id"] in p["final_criteria"]]) or "Current planning delivery",
        })
    for row in packets:
        compare("Execution Packets", "Packet", row["id"], {
            "Module": row["module"], "Outcome": row["title"], "Kind": row["kind"],
            "Scoped predecessor outputs": _list(row["prerequisites"]),
            "Completed items required": _list(row["requires_completed_items"]),
            "Criteria": len(row["criteria"]), "Final criteria": len(row["final_criteria"]),
            "Required choices": _list(row["decisions"]), "Command contracts": _list(row["commands"]),
        })
    for row in commands:
        compare("Command Contracts", "Command", row["id"], {
            "Method": row["method"], "Target path": row["path"], "Owner criteria": _list(row["owner_ac"]),
            "Request schema": _text(row["request_schema"]), "Response schema": _text(row["response_schema"]),
        })
    approval_records = _read(root, "docs/blueprint/approvals.json")["records"]
    for row in decisions:
        matching_adoptions = sorted(record["id"] for record in approval_records
                                    if record["choice"] == row["id"])
        approval_label = ("approval not machine-resolved / manual verification required ("
                          + ", ".join(matching_adoptions) + ")" if matching_adoptions
                          else "no adoption record recorded")
        compare("Decisions", "Choice", row["id"], {
            "Recommendation": row["recommendation"], "Fallback": row["fallback"],
            "Accountable approver": row["approver"], "Approval required for": _list(row["approval_required_for"]),
            "Current approval": approval_label,
        })
    for row in scenarios:
        compare("Evaluation Specs", "Scenario", row["id"], {
            "Module": row["module"], "Purpose": row["title"],
            "Owner criteria": _list(row["owner_criteria"]), "Method": row["method"],
            "Execution": "NOT RUN — specification only",
        })
    for row in modules:
        compare("Modules", "Module", row["id"], {
            "Capability": row["title"], "Context modules": _list(row["requires"]),
            "Work items": _list(row["items"]), "Features": _list(row["features"]),
            "Steps": _list(row["steps"]), "Chapter": "docs/blueprint/" + row["guide"],
        })
    for row in rules:
        compare("Build Assurance", "Rule", row["id"], {
            "Stage": row["stage"], "Kind": row["kind"], "Requirement": row["statement"],
    ...
    for row in plan["wave_contracts"]:
        compare("Delivery Plan", "Wave", row["id"], {
            "Outcome": row["name"] + "\n" + row["goal"], "Scope": _text(row["scope"]),
            "Entry conditions": _text(row["entry_conditions"]), "Exit conditions": _text(row["exit_conditions"]),
            "Release claim limit": _text(row["release_claim"]), "Control": _text(row["control"]),
            "Assigned work": _list([w["id"] for w in plan["item_waves"] if w["wave"] == row["id"]]),
        })
    for row in professional["advocate_standards"]:
        compare("Advocate Standard", "ID", row["id"], {
            "Quality": row["quality"], "Observable behaviour": row["observable"],
            "Trust-destroying failure": row["failure"], "Registered work": _list(row["work_items"]),
            "Features / steps": _list(row["features"]) + "\n" + _list(row["steps"]),
        })
    for row in professional["workflow_states"]:
        compare("Expert Workflow", "ID", row["id"], {
            "Working state": row["state"], "Expert purpose": row["purpose"],
            "Required output": _text(row["required_output"]), "Registered work": _list(row["work_items"]),
            "Features / steps": _list(row["features"]) + "\n" + _list(row["steps"]),
        })
    for row in professional["advice_maturity"]:
        compare("Advice Maturity", "ID", row["id"], {
            "Level": row["level"], "When permitted": _text(row["when_permitted"]),
            "Must contain": _text(row["must_contain"]), "Must not imply": _text(row["must_not_imply"]),
            "Registered work": _list(row["work_items"]),
            "Features / steps": _list(row["features"]) + "\n" + _list(row["steps"]),
        })
    for row in professional["roles"]:
        compare("Roles & Authority", "ID", row["id"], {
            "Role": row["role"], "May do": _text(row["may"]), "Controlled / restricted": _text(row["controlled"]),
            "Registered work": _list(row["work_items"]),
        })
    for row in professional["gap_closures"]:
        compare("Gap Closure", "ID", row["id"], {
            "Gap": row["gap"], "Required plan change": _text(row["plan_change"]),
            "Minimum acceptance": _text(row["minimum_acceptance"]),
            "Registered stage links": _list([link["item"] + " (" + link["stage"] + ")" for link in row["links"]]),
            "Foundation": row["foundation_wave"], "Feature complete": row["feature_complete_wave"],
            "Release gate": row["release_gate_wave"],
        })

    # Check finite instruction populations as well as the one-row indexes.
    guide = [{"Packet": p["id"], "Outcome": p["title"], "Aspect": aspect,
              "Order": index + 1, "Instruction": instruction}
             for p in packets for aspect in ("inputs", "outputs", "steps", "expected", "rollback")
             for index, instruction in enumerate(p[aspect] if isinstance(p[aspect], list) else [p[aspect]])]
    if tables["Packet Guide"] != guide:
        errors.append("Packet Guide: instruction population or content differs")
    step_contracts = [{"Step ID": s["id"], "Phase": s["phase"], "Step": s["name"],
                       "Aspect": label, "Intended contract": _text(s[key]),
                       "Registered work": _list(s["items"])}
                      for s in steps for key, label in CONTRACT_ASPECTS.items() if s.get(key)]
    if tables["Step Contracts"] != step_contracts:
        errors.append("Step Contracts: clause population or content differs")
    evaluation_details = []
    for row in scenarios:
        evaluation_details.extend({"Scenario": row["id"], "Aspect": "Input", "Field / order": key,
                                   "Specification": _text(value)} for key, value in row["inputs"].items())
        evaluation_details.extend({"Scenario": row["id"], "Aspect": "Sequence", "Field / order": i + 1,
                                   "Specification": value} for i, value in enumerate(row["sequence"]))
        evaluation_details.extend({"Scenario": row["id"], "Aspect": "Expected observation",
                                   "Field / order": value["path"],
                                   "Specification": value["operator"] + ": " + _text(value["value"])}
                                  for value in row["expected"])
        evaluation_details.extend({"Scenario": row["id"], "Aspect": "Live observation", "Field / order": i + 1,
                                   "Specification": value} for i, value in enumerate(row["live_observation"]))
    for key, label in (("observation_contract", "Observation contract"), ("media_contract", "Media policy")):
        evaluation_details.extend({"Scenario": "Shared contract", "Aspect": label,
                                   "Field / order": field, "Specification": _text(value)}
    ...
    def autonomy_leaves(value, path=()):
        if isinstance(value, dict):
            ...
            yield {"Scenario": "Autonomy contract", "Aspect": path[0],
                   "Field / order": ".".join(path[1:]) or path[0], "Specification": _text(value)}
    ...
    differing = [i for i, (actual, expected) in enumerate(zip(saved_evaluation_details, evaluation_details), 4)
                 if set(actual) != set(expected) or any(not _same_value(actual.get(k), v)
    ...
    if len(saved_evaluation_details) != len(evaluation_details) or differing:
        ...
        errors.append(f"Evaluation Details: specification population or content differs at row {mismatch}")
    for sheet, key, source in (("Release Gates", "Gate", context["review_gates"]),
    ...
    for index, (label, sheet, _, source, column) in enumerate(groups, 4):
        ...
        compare("Reconciliation", "Population", label, {
            "Source count": len(source), "Workbook count": len(source), "Difference": 0,
        })
        formula("Reconciliation", f"C{index}", f"=COUNTA('{sheet}'!{column}4:{column}{len(source)+3})", len(source))
    ...
    for label, count in (("Release profiles", len(profiles)),
    ...
    for index, phase in enumerate("ABCDEFGHI", 4):
        ...
        expected = [len(fs), *(sum(f["implementation"] == state for f in fs)
                              for state in ("complete", "partial", "none")), len(ss),
                    sum(all(s.get(k) for k in list(CONTRACT_ASPECTS)[:8]) for s in ss)]
        columns = ["Features", "Implemented complete", "Partial", "None", "Steps", "Full intended contracts"]
        compare("Overview", "Phase", phase, dict(zip(columns, expected)))
        for letter, value in zip("CDEFGH", expected):
            record = formulas.get(("Overview", f"{letter}{index}"), {})
            if not isinstance(record.get("formula"), str) or not record["formula"].startswith("=") or record.get("cached") != value:
                errors.append(f"Overview!{letter}{index}: formula or cached value differs")
    for row in formula_rows:
        if row.get("cached") is None or (isinstance(row.get("cached"), str) and row["cached"] in ERROR_VALUES):
    ...
    expected_sources = SNAPSHOT_BASE | {p.relative_to(root).as_posix()
        for p in (root / "docs/blueprint").rglob("*") if p.is_file() and p.suffix in {".md", ".json"}}
    ...
    if (len(captured) != 1 or captured[0].get("Role") != "Captured execution artifact"
        ...
        errors.append("Sources: captured execution artifact must remain historical, not current authority")
    return errors
...
def main() -> int:
    ...
    for error in errors:
        print(f"  FAIL: {error}")
    print(f"Current plan view: {len(SHEETS)} sheets, {len(tables['Work Items'])} items, "
          f"{len(tables['Acceptance'])} criteria; {len(errors)} problems. Read-only; not release proof.")
