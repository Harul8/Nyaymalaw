...
- **BK-31** [A] account access, recovery and workspace identity — *in_progress* · Complete the 21 September Before Build Arrive contract. Pending six-digit confirmation, protected same-device drafts and selective session revocation have focused implementation and verification; profile/account controls, full regression and whole-phase acceptance remain open. Acceptance of the agreement belongs at registration only, never at matter entry. Gmail integration remains disabled by owner instruction; actual mailbox receipt is untested. Preserve D-041 ordinary owned workspace/new/reopen access without professional approval, neutral authentication and separately evidenced authority for protected professional acts. Recovery codes remain removed under F-A-04; email reset is F-A-03. See the 21 September Arrive implementation record in BACKLOG.md. Confidential authentication is BK-86 and deployed assurance is BK-42. Historical evidence does not establish current-source PASS.
...
- **BK-93** [B] opening-route journey and safe risk handoff — *in_progress* · owner parked the remaining opening work on 2026-09-21 for later explicit pickup; media extraction/understanding, action-permission reconciliation, real voice/transcription validation, per-run-approved live-model evaluation and qualified professional/retention review remain OPEN, with integration and cumulative gate refresh still required; current foundation stays partial, not a full B1 acceptance claim; next proposed planning discussion is C Take the brief, not authorisation to build or bypass these dependencies
