...
| 3. Identity and recovery | Public email/password personal accounts, separate invitation/firm binding and professional approval; strong authentication, authenticator lifecycle, rate limiting, session revocation and reviewed recovery | Own workspace/new/reopen do not require professional approval; forged membership/approval and expired/replayed invitation fail. Stolen password alone fails the real-data profile; reused or competing recovery cannot succeed twice | Identity/security; M01 |
...
1. D-041 permits email/password registration without an invitation into a new actor-private workspace. The user can enter that workspace and start, reopen and work owned matters without professional approval. Public signup accepts no firm, role, professional verification or workspace selector, and does not verify mailbox possession. Keep operator invitations as a separate lane bound server-side to intended recipient, firm, role policy, expiry and single use; invalid invitation proof cannot fall through into public signup. An invitation does not itself grant professional approval. Approval is separately attributable, time-bounded and revocable and is checked only on genuinely protected approvals/overrides, alongside—not instead of—current matter and effect authority. Ordinary instructions are not an override. Never use the former private-roster assumption to auto-clear those protected controls.
...
The current protected professional consumer is the creation and use of an
emergency screen-exception declaration. Missing approval must not prevent
ordinary intake, scope/capacity instruction recording, advice or own-file access;
manual urgency recording and revoking unsafe permission remain available.
Approval neither resolves a danger nor grants client/matter/effect authority.
No public request may issue its own professional approval or supply a reviewer
identity as proof. The approval record binds supporting evidence by reference
and digest, reviewer distinct from subject, recorded time, expiry and revocation;
raw qualification documents and internal review detail do not enter public
session projections. A local administrative recording mechanism is not evidence
that an actual qualified review occurred or that deployed operator access is safe.
