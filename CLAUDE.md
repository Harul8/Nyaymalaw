...
| | |
|---|---|
| `docs/Nyaymalaw_PRD.docx`, authored in `spec/prd/` | **The intended product specification.** Regenerate the Word document and `spec/features.yaml` together. Code must be verified against the requirements; the document is not proof that the code conforms. Every feature carries DOES / NEVER / PRODUCES / EVAL |
...
| `docs/PLAN.md`, `docs/Nyaymalaw_End_to_End_Project_Plan.xlsx` | Current reader views. The workbook is generated by `spec/plan/build_current_plan.mjs`; edit the owning registries, not spreadsheet status cells. Profile semantics not yet enforced are explicitly recorded under BK-80 |
