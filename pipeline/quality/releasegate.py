"""The release gate. Measures the corpus, scores it against `assurance/specification/release.yaml`.

    python pipeline/quality/releasegate.py             # measure and score
    python pipeline/quality/releasegate.py --write     # also write
...
`assurance/specification/coverage.yaml`, READ AT TURN TIME by `backend/nm/knowledge/coverage.py`,
and
...
from assurance.common._fingerprint import source_fingerprint  # noqa: E402
...
COVERAGE_OUT = ROOT / "assurance" / "specification" / "coverage.yaml"
...
def measure_judgments() -> dict:
    ...
    return {
        ...
        # and implemented in backend/nm/knowledge/jurisdiction.py.
...
def score(rows: list[dict], m: dict) -> Score:
    ...
    s.add("RG-07", PASS if index.exists() else FAIL,
          ...
          "NOT BUILT -- run python pipeline/indexing/build_authority_index.py",
          blocking("RG-07"))
    ...
    if not identity.exists():
        for rid in ("RG-08", "RG-09"):
            s.add(rid, FAIL, "NOT BUILT -- run python pipeline/indexing/build_identity_index.py",
                  blocking(rid))
    else:
...
def coverage_document(m: dict, s: Score) -> dict:
    ...
    `backend/nm/knowledge/coverage.py` loads this to answer, at turn time, what the
...
def measure_trace() -> dict:
    ...
    """
    try:
        from assurance.gate import trace as tracer
    ...
    if not rec:
        return {"available": False,
                "why": "no mutation run has been recorded since runs began "
                       "carrying a source fingerprint. Run assurance/gate/mutate.py"}
    ...
    if rec.get("source_fingerprint") != now:
        ...
        return {"available": False,
                "why": (f"the recorded mutation run was made against source "
                        ...
                        f"Run assurance/gate/mutate.py")}
...
def measure_goldens() -> dict:
    ...
    """
    try:
        from assurance.journeys import run_goldens as goldens
    except Exception as exc:  # noqa: BLE001
...
def main() -> int:
    ...
    ap.add_argument("--write", action="store_true",
                    help="write assurance/specification/coverage.yaml for the runtime to read")
    args = ap.parse_args()
    ...
    print("=" * 78)
    print("RELEASE GATE   assurance/specification/release.yaml against the measured corpus")
    print("=" * 78)
    for r in s.rows:
        meta = by_id.get(r["id"], {})
    ...
    if args.write:
        ...
        COVERAGE_OUT.write_text(
            "# GENERATED by pipeline/quality/releasegate.py -- do not edit.\n"
            "# MEASURED FACTS ONLY. Thresholds live in assurance/specification/release.yaml, and\n"
            "# backend/nm/knowledge/coverage.py reads this at turn time so the release\n"
