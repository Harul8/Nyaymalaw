"""Recall@k on (matter, governing provision) pairs. E-053, task T-055.
    ...
    python pipeline/quality/recall.py                 # score the sample, if there is one
    python pipeline/quality/recall.py --template      # write an empty sample to fill in
...
`backend/nm/knowledge/resolution.py` would score, the number would be high, and it
...
one. `assurance/specification/release.yaml` RG-19 reads the result, and an absent sample scores
...
SAMPLE = ROOT / "assurance" / "specification" / "recall_sample.json"
...
def main() -> int:
    ...
    if doc is None:
        ...
        print(f"  {result['why']}")
        print("  Run `python pipeline/quality/recall.py --template` to start one.")
