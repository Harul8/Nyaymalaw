...
it. That is the same move `nm/core/accrual.py` makes and for the same reason.
...
SYSTEM = (
    ...
    "THE COMPUTED FACT RULES OUT. Telling them to act within a period the "
    "facts say has run. Telling them to work out something the facts have "
    "already worked out. Advising the move of the party we do not act for. "
    "Asserting an effect the facts record as not computed.\n\n"
    "IT IS NOT A CONTRADICTION FOR THE STEP TO BE ABOUT SOMETHING ELSE. Most "
    "steps are: obtaining a document, writing to the other side, checking a "
    "register. A step that says nothing about a computed fact does not "
...
def repair_prompt(step: str, claim: Claim, why: str, file_note: str = ""):
    ...
    return Prompt(
        system=(
            ...
            "KEEP THE STEP. Same action, same subject, at most 40 words, one "
            "imperative sentence, no preamble and no caveats. You are removing "
            "a contradiction, not writing different advice.\n\n"
            "If the step cannot be stated at all without contradicting the "
            "fact, reply with the step that the fact actually supports.\n\n"
