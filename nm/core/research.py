...
index consulted (by identity), every adverse search, every attachment, the
...
class SupportState(str, Enum):
    ...
    is the third state -- which is why an attachment can be identity-resolved,
...
class Attachment:
    ...
    (`index:locator`). Kept on the attachment so the turn can mark it
...
def fold(text: str) -> str:
    """Whitespace-insensitive, case-preserving. A quotation that differs from
    the source in line breaks is the same words; one that differs in a word
    is not, and no threshold is applied to the difference (CLAUDE.md §5)."""
...
def quote_fidelity(quote: str, source_text: str | None) -> QuoteState:
    ...
    if not q:
        return QuoteState.NOT_CHECKED
    return QuoteState.VERBATIM if q in fold(source_text) else QuoteState.DIFFERS
...
def applicability_of(binding: Binding | None, *, forum_said: str) -> tuple[Applicability, str]:
    ...
    `Binding` is the evidence port's own vocabulary and `nm/knowledge/jurisdiction.py`
...
def research_id(matter_id: str, objective: str, issue: str, at: str) -> str:
    material = f"{matter_id}|{fold(objective)}|{fold(issue)}|{at}"
    return "res_" + hashlib.sha256(material.encode("utf8")).hexdigest()[:12]


def digest_of(text: str) -> str:
    return hashlib.sha256(fold(text).encode("utf8")).hexdigest()[:16]
...
def with_attachment(research: Research, attachment: Attachment) -> Research:
    return replace(research, attachments=(*research.attachments, attachment),
