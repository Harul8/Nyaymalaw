...
in `nm/` permitted to call `unlink` or `rmtree`, and
`tests/test_a_completed_claim_survives_its_own_housekeeping.py` scans the
product and fails the build on a second one.

WHY IT LIVES IN `nm/domain/` AND NOT IN `nm/adapters/store/`
--------------------------------------------------------------
IT LIVED THERE, AND THE LAYER WAS THE DEFECT. `tools/layercheck.py` permits
...
`nm/knowledge/manifest.py` -- it could not have used the one mechanism even had
